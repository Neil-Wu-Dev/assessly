...
public class BenchmarkController {
    ...
    BenchmarkRunResponse run(@RequestHeader("X-Assessly-Session") String token, @Valid @RequestBody BenchmarkRunRequest request) {
        ...
        return mapper.toResponse(benchmarks.run(user.getId(), request.controlText(), request.testDataJson(), request.groundTruthRuleJson(), request.expectedResultJson(), request.aiGeneratedRuleJson()));
    }
}
