  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        <header className="topbar"><div><p className="eyebrow">Status</p><strong>{message}</strong></div><form className="api-connect" onSubmit={connectApi}><span className={`pill ${apiSession.status.toLowerCase()}`}>{apiSession.status}</span><span>{apiSession.expiresAt ? `Expires ${new Date(apiSession.expiresAt).toLocaleTimeString()}` : 'No API session'}</span><input name="providerName" placeholder="Provider name" defaultValue={apiSession.providerName ?? ''} /><input name="baseUrl" placeholder="Provider base URL" defaultValue={apiSession.baseUrl ?? ''} /><input name="modelName" placeholder="Model name" defaultValue={apiSession.modelName ?? ''} /><input name="apiKey" type="password" placeholder="API Key, never stored" /><button>Connect</button></form></header>
        {!detail ? <div className="empty">Create or select a Dataset to begin.</div> : <div className="grid">
          ...
          <section className="panel"><h2>Rule Generation</h2><p className="muted">The selected AI provider translates controls into constrained AST. Final PASS/FAIL remains deterministic.</p><button onClick={generateRules}>Generate Rules</button><RuleSummary rule={activeRule} /></section>
