...
function EvidencePage(props: { detail: Detail; rows: Record<string, unknown>[]; selectedEvidenceId: string; setSelectedEvidenceId: (value: string) => void; upload: (kind: 'controls' | 'evidence', file?: File) => void; deleteEvidence: (id: string) => void }) {
  return <section className="page"><PageHeader eyebrow="Page 3" title="Cybersecurity Evidence Data" subtitle="Upload structured evidence only. Unsupported or unstructured files are rejected by the backend parser." /><UploadCard title="Upload Evidence" description="Supported formats: CSV, XLSX, JSON, JSONL, XML." onFile={(file) => props.upload('evidence', file)} /><div className="split"><div className="card"><h2>Evidence Files</h2><div className="records vertical">{props.detail.evidenceFiles.map((file) => <article key={file.id} className={file.id === props.selectedEvidenceId ? 'selected-record' : ''}><button onClick={() => props.setSelectedEvidenceId(file.id)}><strong>{file.filename}</strong><span>{file.format} · {file.rowCount} rows · {new Date(file.createdAt).toLocaleString()}</span></button><button className="danger-button" onClick={() => props.deleteEvidence(file.id)}>Delete</button></article>)}</div></div><div className="card"><h2>Table Preview</h2><DataPreview rows={props.rows} /></div></div></section>
}
...
function RulesPage(props: { detail: Detail; apiSession: ApiSession; manualRule: string; setManualRule: (value: string) => void; generateRules: () => void; saveManualRules: () => void; confirmRuleSet: (id: string) => void }) {
  return <section className="page"><PageHeader eyebrow="Page 4" title="AI Rule Blocks and Visual Builder" subtitle="Select uploaded controls and evidence context, generate constrained rule AST, review, edit, and confirm." /><div className="rule-layout"><div className="card form-stack"><h2>Generation Inputs</h2><label>AI Session<StatusPill value={props.apiSession.status} /></label><label>Security Control<select>{props.detail.controlDocuments.map((control) => <option key={control.id}>{control.title || control.filename}</option>)}</select></label><label>Evidence Schema<select>{props.detail.evidenceFiles.map((file) => <option key={file.id}>{file.filename}</option>)}</select></label><button className="primary" onClick={props.generateRules}>Generate Rule Blocks</button><div className="builder-row"><select><option>IF</option><option>AND</option><option>OR</option><option>NOT</option></select><select><option>role</option><option>mfa_enabled</option></select><select><option>=</option><option>IN</option><option>CONTAINS</option></select><input defaultValue="admin" /></div><textarea className="code-editor" value={props.manualRule} onChange={(event) => props.setManualRule(event.target.value)} /><button onClick={props.saveManualRules}>Save Manual Rule Set</button></div><div className="card"><h2>Rule Sets</h2><div className="records vertical">{props.detail.ruleSets.map((ruleSet) => <article key={ruleSet.id}><div className="record-header"><strong>Version {ruleSet.version}</strong><StatusPill value={ruleSet.status} /></div><span>{ruleSet.confirmedAt ? `Confirmed ${new Date(ruleSet.confirmedAt).toLocaleString()}` : 'Not confirmed'}</span><pre>{pretty(ruleSet.rulesJson)}</pre><button onClick={() => props.confirmRuleSet(ruleSet.id)}>Confirm Rule Set</button></article>)}</div></div></div></section>
}
...
function AssessmentPage(props: { detail: Detail; selectedEvidenceId: string; setSelectedEvidenceId: (value: string) => void; selectedRuleSetId: string; setSelectedRuleSetId: (value: string) => void; startAssessment: () => void }) {
  return <section className="page"><PageHeader eyebrow="Page 5" title="Start Security Assessment" subtitle="The Java rule engine evaluates confirmed machine rules against selected structured evidence. AI is not called here." /><div className="card form-grid"><label>Evidence file<select value={props.selectedEvidenceId} onChange={(event) => props.setSelectedEvidenceId(event.target.value)}>{props.detail.evidenceFiles.map((file) => <option key={file.id} value={file.id}>{file.filename}</option>)}</select></label><label>Rule Set<select value={props.selectedRuleSetId} onChange={(event) => props.setSelectedRuleSetId(event.target.value)}>{props.detail.ruleSets.map((rule) => <option key={rule.id} value={rule.id}>Version {rule.version} · {rule.status}</option>)}</select></label><button className="primary" disabled={!props.selectedEvidenceId || !props.selectedRuleSetId} onClick={props.startAssessment}>Start Assessment</button></div><div className="card metrics"><Metric label="Available evidence" value={props.detail.evidenceFiles.length} /><Metric label="Available rule sets" value={props.detail.ruleSets.length} /><Metric label="Previous runs" value={props.detail.assessmentRuns.length} /></div></section>
}

function HistoryPage({ runs }: { runs: Assessment[] }) {
  return <section className="page"><PageHeader eyebrow="Traceability" title="Assessment History and Results" subtitle="Every run stays tied to the evidence file and rule version used at execution time." /><div className="card"><h2>Runs</h2><div className="records vertical">{runs.map((run) => <article key={run.id}><div className="record-header"><strong>{new Date(run.createdAt).toLocaleString()}</strong><StatusPill value={run.violationsDetected ? `${run.violationsDetected} VIOLATIONS` : 'PASS'} /></div><span>{run.recordsEvaluated} records · {run.rulesEvaluated} rules · status {run.status}</span><pre>{pretty(run.resultJson)}</pre></article>)}</div></div></section>
}
...
function DataPreview({ rows }: { rows: Record<string, unknown>[] }) {
  if (!rows.length) return <div className="empty-box">No structured evidence uploaded.</div>
